...
For a full end-to-end example including custom context, role checks, and feature flags, follow the [Procurement Workflow tutorial](/docs/tutorials/procurement-workflow/overview) — a 15-model procurement system built on these hooks (its auth chain is `setTokens` → `setUser` → `grantAccess`).
